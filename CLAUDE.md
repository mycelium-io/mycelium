...
- **CFN timeout boundary: mycelium owns agent-waiting, CFN owns compute.** There are three distinct timeouts — do not conflate them:
  - `COORDINATION_TICK_TIMEOUT_SECONDS` (30s): CE per-tick budget. How long the CognitiveEngine waits for a single agent reply before falling back to a safe default.
  - `CFN_ROUND_TIMEOUT_SECONDS` (300s): round-silence watchdog (`coordination.py:_CFN_ROUND_TIMEOUT_SECS`). Fires only when agents go *completely* silent (disconnected/crashed). Restarts on each agent's first reply per round (`_reset_round_timeout`), so serialized runtimes get a fresh budget each reply. Runs *before* `/decide`.
  - `CFN_DECIDE_TIMEOUT_SECONDS` (600s): dead-connection backstop for the HTTP call to CFN `/decide`. Must sit comfortably above the CFN's own internal negotiation timeout so the CFN's structured `status:"timeout"` always wins over an opaque transport error.
  Do NOT collapse these into one session-wide deadline; a flat wall-clock would re-break the single-threaded case the per-reply restart protects.
- **CFN retry/validation-threshold policy is set via a blanket env var on the cognition-engine container, not per-MAS API config.** `retry_max_attempts` (CFN default 3) restarts a negotiation from round 1 on a low alignment score, silently running a session several times over. The per-MAS `config` field on `MultiAgenticSystemRequest` (what `rooms.py:_ensure_mas`/`_sync_create_mas` used to send at MAS creation) persists on the MAS record but is never consulted by the CE for auto-associated MAS entries — its resolved-config lookup 404s and it silently falls through to its own `Settings` class (confirmed live, 2026-07-21; the per-MAS path was removed as dead code). `RETRY_MAX_ATTEMPTS` / `VALIDATION_SCORE_INTERVENTION` on the `ioc-cfn-cognition-engine` service in `compose.yml` (sourced from `config.toml`'s `runtime.cfn_retry_max_attempts`/`cfn_validation_score_intervention` via `.env`) are the only mechanism that actually works — pydantic-settings binds them case-insensitively by field name. Defaults: retry 1 (one restart on a low alignment score before taking the agreement), validation-intervention threshold 0.6. A CFN-internal retry is surfaced as a `coordination_retry` message (detected via round regression in `_cfn_decide_round`).
