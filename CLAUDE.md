...
- **The CFN is the Go ioc-cfn-svc; the python CFN is gone (2.0.0).** All CFN calls (`cfn_negotiation.py` semantic-alignment start/decide, `cfn_knowledge.py` shared-memories + graph reads) are plain httpx against documented JSON shapes — there is no generated CFN client anymore. Decide replies key on `participant_id`; the agreement arrives as a `final_result` envelope. The compose service keeps the historical name `ioc-cognition-fabric-node-svc` (DNS/URL compatibility for existing installs) but runs the `ghcr.io/outshift-open/ioc-cfn-svc` image. Epistemic reply extras (confidence/evidence/deferred_to/reasoning) are parsed and tracked mycelium-side but never forwarded to CFN.
