...
- **The CFN is the Go ioc-cfn-svc; the python CFN is gone (2.0.0).** The env var is `CFN_SVC_URL` (was `COGNITION_FABRIC_NODE_URL`) and the compose service is `ioc-cfn-svc` — the cfn profile runs the full validated 4-service stack (mgmt-plane, knowledge-memory-svc, ioc-cfn-svc, cognition-engine) pinned to a coherent `2026-06-23-*` set. Decide replies key on `participant_id`; the agreement arrives as a `final_result` envelope. Epistemic reply extras (confidence/evidence/deferred_to/reasoning) are parsed mycelium-side but never forwarded to CFN.
