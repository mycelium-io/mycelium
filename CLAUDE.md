...
- **Dual-stack CFN client** — `CFN_API_FLAVOR=negotiation` (default; python CFN, semantic-negotiation API, generated client) or `alignment` (Go CFN ioc-cfn-svc, semantic-alignment API, plain httpx; decide replies key on `participant_id`, agreement arrives as a `final_result` envelope). Config: `runtime.cfn_api_flavor`. Epistemic reply extras (confidence/evidence/deferred_to/reasoning) are parsed and tracked mycelium-side but never forwarded to CFN.
