...
def test_episode_record_carries_a_thread_of_its_own(tmp_path, monkeypatch):
    # Episode records get their own thread binding on write.
    monkeypatch.setenv("MYCELIUM_DATA_DIR", str(tmp_path))
    from app.config import settings
...
def test_rule_update_keeps_one_thread_across_its_rewrites(tmp_path, monkeypatch):
    # The rule memory is rewritten in place on every convergence; its thread
    # binding stays stable across rewrites.
    monkeypatch.setenv("MYCELIUM_DATA_DIR", str(tmp_path))
    from app.config import settings
