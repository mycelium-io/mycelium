...
async def index_single_file(room_name: str, key: str, db: AsyncSession) -> bool:
    ...
    # Stale-room protection: if the room's directory survives on disk but the
    # room row is gone from the DB (e.g. after a volume/DB wipe), indexing a
    # Memory row would violate the FK and poison the session, cascading into a
    # PendingRollbackError for every subsequent watcher event. Skip instead.
...
async def _index_single_memory(
    *,
    ...
    # Atomic upsert on the (room_name, key) unique constraint. Replaces the
    # prior find-then-update/insert, which raced under concurrent writers and
    # could leave the session in a rolled-back state on constraint violation.
    stmt = (
        pg_insert(Memory)
        .values(
            room_name=room_name,
