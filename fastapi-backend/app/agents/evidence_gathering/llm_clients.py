...
class EvidenceJudge(_LLMBaseClient):
    ...
    def select_paths_and_check_sufficiency(
        ...
        try:
            ...
            selected = [i for i in (data.get("selected") or []) if isinstance(i, int) and 0 <= i < len(candidate_paths)]
            sufficient = bool(data.get("sufficient", False))
            reason = str(data.get("reason") or "").splitlines()[0].strip() if data.get("reason") else ""
...
class EntityExtractor(_LLMBaseClient):
    ...
    STOPWORDS = {"what", "does", "do", "the", "a", "an", "and", "or", "to", "of", "for", "in", "on", "with", "is", "are"}
    ...
    def extract_entities_from_request(self, request: object) -> List[Dict]:
        ...
        for rec in getattr(getattr(request, "payload", None), "records", None) or []:
            try:
                rt = rec.record_type.value if hasattr(rec, "record_type") and hasattr(rec.record_type, "value") else str(getattr(rec, "record_type", ""))
            except Exception:
                rt = ""
        ...
        try:
            ...
            return [{"name": e.get("name", "").strip()} for e in ents if isinstance(e, dict) and e.get("name")]
        except Exception:
            return self._fallback_extract(intent, texts)
    ...
    def decompose(self, text: str, entities: List[str] | None = None) -> List[Dict]:
        ...
        for line in (content or "").splitlines():
            ...
            if not line or not line.startswith("#"):
                continue
            parts = line.split("##")
            parsed_entities = [parts[i].strip() for i in range(1, len(parts), 2) if parts[i].strip()][:2]
            sent_part = parts[0]
            ...
            out.append({"index": len(out) + 1, "sentence": sent_part.strip(), "entities": parsed_entities})
