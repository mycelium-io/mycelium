...
"""
Assignment API — who holds a row, and for how much longer.

GET  /rooms/{room}/assignments/{key}        — one row's assignment, right now
POST /rooms/{room}/assignments/claim        — take assignment, for a window
POST /rooms/{room}/assignments/release      — hand it back, with a signed note
POST /rooms/{room}/assignments/resolve      — the work is done; the row leaves
POST /rooms/{room}/assignments/renew        — a resident loop's heartbeat
GET  /rooms/{room}/assignments/await        — wake on this lease's transitions
...
from app.services import actor, assignments
from app.services.filesystem import room_exists

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/rooms/{room_name}/assignments", tags=["assignments"])
...
def _fail(error: assignments.AssignmentError) -> HTTPException:
...
class ClaimBody(BaseModel):
    ...
    handle: str = Field(..., description="The handle taking assignment")
    ttl_minutes: int = Field(
        assignments.DEFAULT_TTL_MINUTES,
        ge=1,
...
@router.post("/claim")
async def claim_assignment(room_name: str, body: ClaimBody, request: Request):
    """Take assignment of a row for a window, not for good."""
...
@router.post("/release")
async def release_assignment(room_name: str, body: ReleaseBody, request: Request):
    """Hand a row back on purpose, and sign the note that says so."""
    _require_room(room_name)
    handle = actor.bind_delegated_actor(request, room_name, body.handle, field="handle")
    try:
        return await assignments.release(room_name, body.key, handle, body.note, datetime.now(UTC))
    except assignments.AssignmentError as e:
        raise _fail(e) from e


@router.post("/resolve")
async def resolve_assignment(room_name: str, body: ResolveBody, request: Request):
    """The work is done: the row leaves the board rather than draining out of it."""
    _require_room(room_name)
    handle = actor.bind_delegated_actor(request, room_name, body.handle, field="handle")
    try:
        return await assignments.resolve(room_name, body.key, handle, datetime.now(UTC))
    except assignments.AssignmentError as e:
        raise _fail(e) from e


@router.post("/renew")
async def renew_assignments(room_name: str, body: RenewBody, request: Request):
    ...
    _require_room(room_name)
    handle = actor.bind_delegated_actor(request, room_name, body.handle, field="handle")
    renewed = await assignments.renew(room_name, handle, datetime.now(UTC))
...
async def await_assignment(
    ...
    timeout: int = Query(
        0, ge=0, le=assignments.MAX_WAIT_S, description="Seconds to hold the request"
    ),
):
    """Wake on this lease's transitions, and on nothing else."""
    _require_room(room_name)
    try:
        return await assignments.watch(room_name, key, since, timeout)
    except assignments.AssignmentError as e:
        raise _fail(e) from e


@router.get("/{key:path}")
async def read_assignment(room_name: str, key: str):
    """One row's assignment as it stands, including a drain nobody wrote down."""
    _require_room(room_name)
    try:
        return assignments.read(room_name, key, datetime.now(UTC))
    except assignments.AssignmentError as e:
        raise _fail(e) from e
