...
def upgrade() -> None:
    ...
    if not rows:
        logger.info("0019: no mixed-case room names found, nothing to do")
        return
    ...
    # messages.room_name, memories.room_name, and memory_subscriptions.room_name.
    # They declare ON DELETE CASCADE but NOT ON UPDATE CASCADE, so a bare
    # `UPDATE rooms SET name = ...` would be rejected while child rows still
    # reference the old name. Add ON UPDATE CASCADE to every FK pointing at
    # rooms(name), then a single parent update cascades to all children (present
    # and future) in one shot. ON UPDATE CASCADE is the correct long-term
    # semantics for these ownership FKs, so it is left in place.
    ...
    if fs_rename_failed:
        ...
        safe_rows = [
            (room_id, old_name) for room_id, old_name in rows if old_name not in fs_rename_failed
        ]
