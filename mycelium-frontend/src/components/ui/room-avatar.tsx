 ...
 *  The color comes from the name, so it needs no storage and never drifts
 *  between the rail, the command center and a card.
 *
 *  Selection is carried by the surrounding row (its own background + ring), not
 *  the tile, so the identity color reads the same open or not.
 *
...
export function RoomAvatar({ name, className, children }: Props) {
  ...
  return (
    <span
      ...
      // Solid, opaque fill so the tile keeps its color over a hover or
