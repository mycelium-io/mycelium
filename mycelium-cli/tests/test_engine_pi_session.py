...
"""The Pi session's custom-endpoint handling (LLM_BASE_URL → pi models.json).
...
def test_prompt_leading_at_and_dash_are_neutralized(tmp_path: Path) -> None:
    ...
    assert (
        _llm_session(tmp_path)._build_command("@growth propose.", system="")[-1]
        == " @growth propose."
    )
    assert _llm_session(tmp_path)._build_command("-5 counter.", system="")[-1] == " -5 counter."
    assert _llm_session(tmp_path)._build_command("propose.", system="")[-1] == "propose."
