...
def test_tokens_by_channel_disjoint_channels_per_host() -> None:
    ...
    _push_tokens(store, host="oclw-3", channel="matrix", input_tokens=100, output_tokens=10)
    _push_tokens(store, host="oclw-4", channel="cfn", input_tokens=200, output_tokens=20)
    _push_tokens(store, host="oclw-5", channel="matrix", input_tokens=300, output_tokens=30)

    by_agent = store.to_dict()["counters"]["tokens"]["by_agent"]
    assert by_agent["matrix"]["input"] == 100 + 300
    assert by_agent["matrix"]["output"] == 10 + 30
...
def test_tokens_repushed_from_same_host_overwrites_within_host() -> None:
    ...
    _push_tokens(store, host="oclw-3", channel="matrix", input_tokens=100)
    _push_tokens(store, host="oclw-4", channel="matrix", input_tokens=200)
    # oclw-3 pushes again with a larger cumulative value.
    _push_tokens(store, host="oclw-3", channel="matrix", input_tokens=150)

    by_agent = store.to_dict()["counters"]["tokens"]["by_agent"]
    assert by_agent["matrix"]["input"] == 150 + 200
...
def test_cost_usd_sums_across_hosts() -> None:
    ...
    _push_cost(store, host="oclw-3", channel="matrix", value=1.25)
    _push_cost(store, host="oclw-5", channel="matrix", value=0.75)

    snap = store.to_dict()
    assert snap["counters"]["cost_usd"]["by_agent"]["matrix"] == pytest.approx(2.00)
...
def test_snapshot_exposes_counters_by_host_for_persistence() -> None:
    ...
    store = MetricsStore()
    _push_tokens(store, host="oclw-3", channel="matrix", input_tokens=10)
    _push_tokens(store, host="oclw-5", channel="matrix", input_tokens=20)

    snap = store.to_dict()
    assert "counters_by_host" in snap
    assert set(snap["counters_by_host"].keys()) == {"oclw-3", "oclw-5"}
    assert snap["counters_by_host"]["oclw-3"]["tokens"]["by_agent"]["matrix"]["input"] == 10
    assert snap["counters_by_host"]["oclw-5"]["tokens"]["by_agent"]["matrix"]["input"] == 20
...
def test_unknown_host_does_not_clobber_known_hosts() -> None:
    ...
    _push_tokens(store, host="oclw-3", channel="matrix", input_tokens=500)
    _push_tokens(store, host="", channel="matrix", input_tokens=42)

    snap = store.to_dict()
    assert snap["counters_by_host"]["oclw-3"]["tokens"]["by_agent"]["matrix"]["input"] == 500
    assert snap["counters_by_host"]["unknown"]["tokens"]["by_agent"]["matrix"]["input"] == 42
    assert snap["counters"]["tokens"]["by_agent"]["matrix"]["input"] == 500 + 42
