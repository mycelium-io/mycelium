...
export function clearSubscribedSessions() {
...
export function stopSessionSSE(sessionRoom, log) {
...
export function subscribedSessionRooms() {
    return new Set(_sessions.keys());
}
export function startSessionSSE(runtime, cfg, sessionRoom, _gatewayAbort, handleMessage, log) {
    if (_sessions.has(sessionRoom))
        return;
    ...
    (async () => {
        ...
        while (!signal.aborted) {
            try {
                ...
                if (res.status === 404) {
                    log.info(`[${CHANNEL_ID}] session room gone (404): ${sessionRoom} — unsubscribing`);
                ...
                if (!res.ok || !res.body) {
                    ...
                    if (transientFailures >= MAX_TRANSIENT_RETRIES) {
                        log.warn(`[${CHANNEL_ID}] session SSE gave up after ${MAX_TRANSIENT_RETRIES} failures for ${sessionRoom}`);
                        stopSessionSSE(sessionRoom);
                        return;
                    }
                    const backoff = Math.min(5000 * 2 ** (transientFailures - 1), 30_000);
                    log.warn(`[${CHANNEL_ID}] session SSE ${res.status} for ${sessionRoom} — retry ${transientFailures}/${MAX_TRANSIENT_RETRIES} in ${backoff}ms`);
                ...
                transientFailures++;
                if (transientFailures >= MAX_TRANSIENT_RETRIES) {
                    log.warn(`[${CHANNEL_ID}] session SSE gave up after ${MAX_TRANSIENT_RETRIES} errors for ${sessionRoom}`);
                    stopSessionSSE(sessionRoom);
                    return;
                }
                const backoff = Math.min(5000 * 2 ** (transientFailures - 1), 30_000);
                log.warn(`[${CHANNEL_ID}] session SSE error: ${err?.message} — retry ${transientFailures}/${MAX_TRANSIENT_RETRIES} in ${backoff}ms`);
                await new Promise((r) => setTimeout(r, backoff));
            }
        }
    })();
