 ...
 */
export function readHomeAddress(agentId, log) {
 ...
 */
export function stashReturnAddress(sessionRoom, agentId, log) {
    ...
    if (!addr) {
        log.info(`[mycelium-room] return-address: no home channel for ${agentId} on join — will skip notify-home`);
        return;
    }
    perAgent.set(agentId, addr);
    log.info(`[mycelium-room] return-address stashed: ${agentId} → ${addr.channelName}:${addr.to}`);
}
export function lookupReturnAddress(sessionRoom, agentId) {
    return _stash.get(stashKey(sessionRoom))?.get(agentId);
}
export function clearReturnAddresses(sessionRoom) {
    _stash.delete(stashKey(sessionRoom));
}
/** Test/debug helper. */
export function _allReturnAddresses() {
    return _stash;
}
