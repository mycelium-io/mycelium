...
export async function fetchAgentContext(backendUrl, room, handle) {
    const now = Date.now();
    const cached = _cache.get(room);
    if (cached && now - cached.fetchedMs < TTL_MS)
        return cached;
    try {
        const url = `${backendUrl}/api/rooms/${encodeURIComponent(room)}/agent-context` +
...
export function humanizeAge(generatedAtIso) {
    if (!generatedAtIso)
        return "";
...
export function renderPlanBlock(context, generatedAtIso) {
    if (!context)
        return "";
