...
export const MEMORY_FILE = join(homedir(), ".openclaw/workspace/memory/mycelium-context.md");
...
export function loadMyceliumConfig() {
...
export function getMasId() {
    return masId;
}
export function getAgentId() {
    return process.env.MYCELIUM_AGENT_ID ?? "";
}
export function resolveHandle(agentId) {
...
export function readMemoryFileContent() {
    try {
 ...
 */
export function readChannelConfigs(openclawConfig) {
    ...
    if (Array.isArray(entry.rooms)) {
        for (const r of entry.rooms) {
            ...
            if (seen.has(room))
                continue; // de-dupe against the top-level room
            seen.add(room);
            out.push({
                backendUrl,
                room,
 ...
 */
export function readChannelConfig(openclawConfig) {
