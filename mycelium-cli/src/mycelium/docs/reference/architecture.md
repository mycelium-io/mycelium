...
reads directly (compose already bind-mounts `~/.mycelium` into the container). An
environment variable of the same name wins over the file, so a container that
injects one `GITHUB_TOKEN` and carries no store file works with nothing further.
