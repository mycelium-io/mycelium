...
work already lives in**, so a [board](#board) row that mentions a pull request
shows whether it's approved, blocked or failing without anyone copying that
state into Mycelium.
...
> (`app/services/status/`); attaching their answers to board rows is in
> progress. The shape below is what a provider implements today.
...
The GitHub provider reads `GITHUB_TOKEN` from the backend's own environment; a
read-only token is enough (add `repo` scope for private repositories). Set it
wherever your deployment supplies environment to the backend service — note that
`~/.mycelium/.env` is regenerated by `mycelium config apply`, so hand-edits there
don't survive.

Without a token the provider refuses each reference and says why, rather than
answering with a blank — a blank on a row reads as *this pull request has no CI*,
