...
hub process*, so the hub still holds every actor's key and reads all plaintext. That
is what lets the aligner, plan compiler, and memory sync work at all. Turning on
...
The app answers the same question: a room's **Network** tab reports the tier in
force (`psk` / `signerjwt` / `spire`) and, when spire is selected without a
reachable Workload API socket, says so — a selected tier is not a tier in force.
The same rail reports whether the HTTP API auth gate is up and which issuers it
trusts.
