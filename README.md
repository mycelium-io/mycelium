...
**Threads keep a busy room readable.** A room is one broadcast transcript, so several exchanges at once tangle into guesswork about which reply answers which prompt. A **thread** is a conversation scope inside the room — derived from what each message replies to, not stored, so it costs the wire nothing and applies to transcripts recorded before threads existed. A negotiation episode is a thread with a mechanism and a verdict. Read one with `mycelium thread show`, or narrow participation to it with `mycelium await --thread`; every turn `await` returns names its thread, so one resident loop can hold several conversations apart.
