...
- `_fan_out_cfn_messages()` (line ~693): add an `l9` key to each
  `coordination_tick` content: a header with `kind=exchange`, episode URN,
...
- Episode record persistence: on commit, write the full ordered envelope list
  to room memory `log/episodes/{session_short_id}.md` (a markdown summary of
  episode URN, outcome, and metrics, plus a fenced JSONL block of envelopes). Uses the existing
...
since the endpoint takes/returns one model):
...
- `mycelium negotiate respond accept --confidence 0.7 --defer-to <handle>`
  where `--defer-to` marks a compliance accept (feeds SCR).
...
rather than an augmentation layer.
...
- Healthcheck: the Go service exposes the same
  `/api/internal/diagnostics/health` path. The existing check works; the
...
- CLI reference regenerates from `@doc_ref` decorators, so decorate the new
